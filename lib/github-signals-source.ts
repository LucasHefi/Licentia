import { trackedPublicLicenses, GITHUB_SIGNALS_SOURCE_URL } from "./public-signals.ts";
...
export type GithubSignalsEnvelope = {
  source: string;
  sourceUrl: string;
  fetchedAt: string;
  coverage: string;
...
export async function fetchGithubSignals(): Promise<GithubSignalsEnvelope> {
  const signals = await Promise.all(trackedPublicLicenses.map(([label, query]) => loadLicenseSignal(label, query)));
  ...
  return {
    ...
    coverage: "Veřejné repozitáře, u kterých GitHub rozpoznal licenci; výchozí vyhledávání bez forků.",
