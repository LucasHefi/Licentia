...
function result(id: unknown, value: unknown) { return Response.json({ jsonrpc: "2.0", id, result: value }, { headers: { "Cache-Control": "no-store", "Access-Control-Allow-Origin": "*" } }); }
function failure(id: unknown, code: number, message: string) { return Response.json({ jsonrpc: "2.0", id, error: { code, message } }, { status: code === -32603 ? 500 : 400, headers: { "Cache-Control": "no-store", "Access-Control-Allow-Origin": "*" } }); }
...
export async function POST(request: Request) {
  ...
  try { rpc = await request.json(); } catch { return failure(null, -32700, "Parse error"); }
  const id = rpc.id ?? null;
  try {
    if (rpc.method === "initialize") return result(id, { protocolVersion: "2025-03-26", capabilities: { tools: { listChanged: false }, resources: {} }, serverInfo: { name: "licentia", version: "1.0.0" }, instructions: "Kanonická znění pocházejí ze SPDX. Doporučení jsou orientační a nejsou právní radou." });
    if (rpc.method === "notifications/initialized") return new Response(null, { status: 202 });
    if (rpc.method === "ping") return result(id, {});
    if (rpc.method === "tools/list") return result(id, { tools });
    if (rpc.method === "resources/templates/list") return result(id, { resourceTemplates: [{ uriTemplate: "spdx://licenses/{id}", name: "SPDX licence", mimeType: "application/json" }, { uriTemplate: "spdx://exceptions/{id}", name: "SPDX výjimka", mimeType: "application/json" }] });
    const origin = new URL(request.url).origin;
    if (rpc.method === "resources/read") {
      const match = rpc.params?.uri?.match(/^spdx:\/\/(licenses|exceptions)\/(.+)$/);
      if (!match) return failure(id, -32602, "Neplatné resource URI");
    ...
    return result(id, content(value));
  } catch (error) { return failure(id, -32603, error instanceof Error ? error.message : "Internal error"); }
}

export function GET() { return new Response("Licentia MCP používá Streamable HTTP POST.", { status: 405, headers: { Allow: "POST, OPTIONS" } }); }
