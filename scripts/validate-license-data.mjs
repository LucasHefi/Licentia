...
export function validateProfile(profile, { file = '<profile>', vocabulary = validateContract().vocabulary } = {}) {
  ...
  profile.evidence.forEach((item, index) => { const at = `$.evidence[${index}]`; requireObject(item, at); exactKeys(item, ['field', 'sourceId', 'locator'], at); requireString(item.field, `${at}.field`); requireString(item.sourceId, `${at}.sourceId`); requireString(item.locator, `${at}.locator`); if (!fields.includes(item.field) && item.field !== 'review') fail(`${at}.field: unknown evidence field`); });
